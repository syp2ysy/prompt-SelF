...
import models_mae
from prompt_self_utils import get_query_mask_position
...
def generate_mask_for_evaluation(arrangement):
    mask = np.ones((14, 14))

    quadrants = {
        'top-left': (slice(0, 7), slice(0, 7)),
        'top-right': (slice(0, 7), slice(7, 14)),
        'bottom-left': (slice(7, 14), slice(0, 7)),
        'bottom-right': (slice(7, 14), slice(7, 14)),
    }

    position = get_query_mask_position(arrangement)
    y_slice, x_slice = quadrants[position]
...
def generate_raw_prediction(device, ids_shuffle, len_keep, model, orig_image):
    ...
    for block_num, blk in enumerate(model.decoder_blocks):
        ...
        x = x + blk.drop_path1(blk.mlp(blk.norm2(x)))
